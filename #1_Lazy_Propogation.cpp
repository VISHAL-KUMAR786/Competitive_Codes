/*
    TODO: Lazy Propogation Segment Tree
    ? 
*/
...
public:
    ...
    void update(int low, int high, int ind, int left, int right, int val,
        vector<int> arr) {
        // update previous remaning update & propogate downward
        // # of node responsile (high - low + 1)
        if(lazy[ind] != 0) {
    ...
    int query(int low, int high, int ind, int left, int right) {
        // if prev update, do it & propogate down
        // update if any update are remaining
        // as node will stay fresh & updated
